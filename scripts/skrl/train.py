...
def main(
    ...
    # 记录成功率等自定义指标到 TensorBoard
    _env_step_orig = env.step

    def env_step_with_logging(actions):
        obs, reward, terminated, truncated, info = _env_step_orig(actions)
        # 当有环境结束时，统计结束环境的指标
        # skrl 的 terminated/truncated 形状是 (num_envs, 1)，需要 squeeze 成 (num_envs,)
        dones = (terminated | truncated).squeeze(-1)
        if dones.any():
            if "success" in info:
                # 计算当前结束环境的平均成功率
                # 确保 info["success"] 也是 1D 张量以匹配 dones
                success_tensor = info["success"].reshape(-1)
                success_rate = success_tensor[dones].float().mean().item()
                runner.agent.track_data("Metrics/Success_Rate", success_rate)
            if "collided" in info:
                collision_tensor = info["collided"].reshape(-1)
                collision_rate = collision_tensor[dones].float().mean().item()
                runner.agent.track_data("Metrics/Collision_Rate", collision_rate)
            if "time_out" in info:
                timeout_tensor = info["time_out"].reshape(-1)
                timeout_rate = timeout_tensor[dones].float().mean().item()
                runner.agent.track_data("Metrics/Timeout_Rate", timeout_rate)
        return obs, reward, terminated, truncated, info

    env.step = env_step_with_logging
