...
class CustomCombinedExtractor(BaseFeaturesExtractor):
    ...
    在 share_features_extractor=False，并且 Actor/Critic 分别使用
    pi_features_extractor 和 vf_features_extractor 时，本提取器不参与前向计算。
    """

    def __init__(self, observation_space: gym.spaces.Dict):
        # SB3 要求 features_dim > 0
        super().__init__(
    ...
    def forward(
        ...
        raise RuntimeError(
            "CustomCombinedExtractor 是占位提取器，不应该被调用。"
            "请检查 share_features_extractor=False 以及 Actor/Critic 提取器配置。"
        )
...
class ActorFeaturesExtractor(BaseFeaturesExtractor):
    ...
    def forward(self, observations) -> torch.Tensor:
        ...
        # print("actor特征提取器的forward方法被调用了，img_features.shape:", img_features.shape, "state_features.shape:", state_features.shape)
