...
from stable_baselines3.common.buffers import DictReplayBuffer, NStepReplayBuffer, ReplayBuffer,ExpertDictReplayBuffer
...
SelfOffPolicyAlgorithm = TypeVar("SelfOffPolicyAlgorithm", bound="ExpertOffPolicyAlgorithm")


class ExpertOffPolicyAlgorithm(BaseAlgorithm):
    """
    ...
    def __init__(
        ...
        expert_env=MinimalEnv()
        checkpoint_path="/home/user/isaac_lab_tutorial3/model.zip"
        self.expert_agent=SAC.load(checkpoint_path, expert_env, print_system_info=False)
    ...
    def _setup_model(self) -> None:
        ...
        if self.replay_buffer_class is None:
            if isinstance(self.observation_space, spaces.Dict):
                self.replay_buffer_class = ExpertDictReplayBuffer
    ...
    def collect_rollouts(
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            ...
            expert_last_obs=self._last_obs.copy()
            del expert_last_obs["camera"]
            expert_unscaled_actions,_=self.expert_agent.predict(expert_last_obs, deterministic=True)
            expert_scaled_actions=self.policy.scale_action(expert_unscaled_actions)
            ...
            self._store_transition(replay_buffer, buffer_actions, new_obs, rewards, dones, infos,expert_action=expert_scaled_actions)  # type: ignore[arg-type]
...
class MinimalEnv(gym.Env):
    ...
    def __init__(self):
        ...
        low=np.array([-0.1, -np.pi/6,-0.5], dtype=np.float32),   # 每个维度的最小值
        high=np.array([1.0, np.pi/6,0.5], dtype=np.float32),   # 每个维度的最大值
        shape=(3,),  # 动作空间的形状，2表示两个连续动作
        dtype=np.float32
        )
        # camera_box = spaces.Box(low=-1, high=1, shape=(3,16,12), dtype=np.float32)
        # robot_state_box = spaces.Box(low=-np.inf, high=np.inf, shape=(4,), dtype=np.float32)##########
        # self.num_envs=1
        # # 构造字典
        # my_dict = spaces.Dict({
        #     'camera': camera_box,
        #     'robot-state': robot_state_box
        # })        
        self.observation_space =spaces.Dict({
        "shoot-state": spaces.Box(low=-np.inf, high=np.inf, shape=(10,), dtype=np.float32),
        "robot-state": spaces.Box(low=-np.inf, high=np.inf, shape=(8,), dtype=np.float32),
    })    
        self.state = None
        # 初始化随机数生成器
    ...
    def step(self, action):
        ...
        terminated = self.np_random.random() < 0.01  # 1%概率终止
        truncated = False  # 先设为False，表示非超时结束
        info = {}  # 信息字典
