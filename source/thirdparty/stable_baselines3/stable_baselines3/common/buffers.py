...
class ExpertReplayBuffer(BaseBuffer):
    """
    Replay buffer used in off-policy algorithms like SAC/TD3.
    ...
    def __init__(
        ...
        if psutil is not None:
            total_memory_usage: float = (
                self.observations.nbytes + self.actions.nbytes + self.rewards.nbytes + self.dones.nbytes + self.expert_actions.nbytes
            )
...
class ExpertDictReplayBuffer(ExpertReplayBuffer):
    """
    ExpertDict Replay buffer used in off-policy algorithms like SAC/TD3.
    Extends the ReplayBuffer to use dictionary observations
    ...
    def __init__(
        ...
        super(ExpertReplayBuffer, self).__init__(buffer_size, observation_space, action_space, device, n_envs=n_envs)

        assert isinstance(self.obs_shape, dict), "DictReplayBuffer must be used with Dict obs space only"
        self.buffer_size = max(buffer_size // n_envs, 1)
        #print("Initialized ExpertDictReplayBuffer with buffer size:", self.obs_shape)
        # Check that the replay buffer can fit into the memory
        if psutil is not None:
            mem_available = psutil.virtual_memory().available
        ...
        if psutil is not None:
            ...
            total_memory_usage: float = obs_nbytes + self.actions.nbytes + self.rewards.nbytes + self.dones.nbytes + self.expert_actions.nbytes
            if not optimize_memory_usage:
                next_obs_nbytes = 0
                for _, obs in self.observations.items():
                    next_obs_nbytes += obs.nbytes
    ...
    def add(  # type: ignore[override]
        ...
        for key in self.observations.keys():
            ...
            if isinstance(self.observation_space.spaces[key], spaces.Discrete):
                obs[key] = obs[key].reshape((self.n_envs,) + self.obs_shape[key])
            #print("Adding observation for key:", type(key), )
            #print("obs[key] shape:", obs)
            self.observations[key][self.pos] = np.array(obs[key])
    ...
    def sample(  # type: ignore[override]
        ...
        :return:
        """
        return super(ExpertReplayBuffer, self).sample(batch_size=batch_size, env=env)
    ...
    def _get_samples(  # type: ignore[override]
        ...
        return ExpertDictReplayBufferSamples(
            observations=observations,
            actions=self.to_torch(self.actions[batch_inds, env_indices]),
            expert_actions=self.to_torch(self.expert_actions[batch_inds, env_indices]),
            next_observations=next_observations,
            # Only use dones that are not due to timeouts
            # deactivated by default (timeouts is initialized as an array of False)
            dones=self.to_torch(self.dones[batch_inds, env_indices] * (1 - self.timeouts[batch_inds, env_indices])).reshape(
