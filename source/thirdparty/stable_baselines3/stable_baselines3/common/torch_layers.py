...
class NatureCNN(BaseFeaturesExtractor):
    ...
    def __init__(
        ...
        self.cnn = nn.Sequential(
            nn.Conv2d(n_input_channels, 32, kernel_size=(2,2), stride=1, padding=0),  
            nn.LeakyReLU(0.05),
    ...
    def __init__(
        ...
        for key, subspace in observation_space.spaces.items():
            if is_image_space(subspace, normalized_image=normalized_image):
                ...
                flattened_dim = get_flattened_obs_dim(subspace)
                extractors[key] = nn.Sequential(
                    nn.Flatten(),
                    nn.Linear(flattened_dim, 192),  # 添加192维全连接层
                )
                total_concat_size += 192
                print("mlp")
...
class TeacherCombinedExtractor(BaseFeaturesExtractor):
    """
    Combined features extractor for Dict observation spaces.
    Builds a features extractor for each key of the space. Input from each space
    is fed through a separate submodule (CNN or MLP, depending on input shape),
    ...
    def __init__(
        ...
        for key, subspace in observation_space.spaces.items():
            if is_image_space(subspace, normalized_image=normalized_image):
                ...
                total_concat_size += flattened_dim
                print("mlp")
                print(normalized_image)
        self.extractors = nn.ModuleDict(extractors)
...
class MY_CombinedExtractor(BaseFeaturesExtractor):
    """
    用于字典观测空间的组合特征提取器。
    为空间中的每个键构建特征提取器。每个空间的输入
    通过单独的子模块（根据输入形状使用CNN或MLP）处理，
    输出特征被连接并输入到额外的MLP网络（"组合"网络）中。

    :param observation_space: 观测空间
    :param cnn_output_dim: 从每个CNN子模块输出的特征数量。默认为192，避免网络过大
    :param normalized_image: 是否假设图像已经归一化
        或者没有（这会禁用数据类型和边界检查）：当为True时，只检查空间是否为Box并有3个维度。
        否则，它会检查预期的数据类型（uint8）和边界（值在[0, 255]范围内）。
    """
    ...
    def __init__(
        ...
        # TODO: 在遍历所有项目之前，我们不知道features-dim，所以放一些东西。这有点脏！
        super().__init__(observation_space, features_dim=1)

        extractors: dict[str, nn.Module] = {}
        
        # 初始化特征维度变量
        camera_features_dim = 0
        shoot_state_dim = 0
        robot_state_dim = 192
        total_concat_size = 0
        ...
        for key, subspace in observation_space.spaces.items():
            if is_image_space(subspace, normalized_image=normalized_image):
                ...
                # with th.no_grad():
                #     # 获取CNN输出的特征维度
                #     n_flatten = extractors[key].forward(th.as_tensor(subspace.sample()[None]).float()).shape[1]
                camera_features_dim = 192
                
            elif key == "robot-state":
                # 机器人状态观测键是一个向量，如果需要则展平
                flattened_dim = get_flattened_obs_dim(subspace)
                extractors[key] = nn.Sequential(
                    nn.Flatten(),
                    nn.Linear(flattened_dim, 192),  # 添加192维全连接层
                )
                # robot_state_dim 已经设置为192
                
            elif key == "shoot-state":
                # # 射击状态观测键是一个向量，如果需要则展平
                # flattened_dim = get_flattened_obs_dim(subspace)
                # # 只展平，这里不添加线性层
                # extractors[key] = nn.Sequential(
                #     nn.Flatten(),
        ...
        # 创建线性层来处理拼接后的相机和射击状态特征
        # 输入维度 = 相机特征维度 + 射击状态维度

        # 计算总拼接大小：相机+ + 机器人状态输出
        total_concat_size = 192 + robot_state_dim  # 192 + 192 = 384
    ...
    def forward(self, observations: TensorDict) -> th.Tensor:
        ...
        for key, extractor in self.extractors.items():
            if key == "camera":
                # 这是相机观测
                camera_features = extractor(observations[key])
            elif key == "robot-state":
                # 机器人状态特征
                robot_state_features = extractor(observations[key])
        
        final_features = th.cat([camera_features, robot_state_features], dim=1)

        return final_features

class MY_Assist_CombinedExtractor(BaseFeaturesExtractor):
    """
    Combined features extractor for Dict observation spaces.
    Builds a features extractor for each key of the space. Input from each space
    is fed through a separate submodule (CNN or MLP, depending on input shape),
    the output features are concatenated, then passed through a linear layer to map to 192 dimensions.
    ...
    def __init__(
        ...
        # TODO we do not know features-dim here before going over all the items, so put something there. This is dirty!
        super().__init__(observation_space, features_dim=9856)
        ...
        for key, subspace in observation_space.spaces.items():
            if is_image_space(subspace, normalized_image=normalized_image):
                extractors[key] = MY_Assist_NatureCNN(subspace, features_dim=cnn_output_dim, normalized_image=normalized_image)
                with th.no_grad():
                    n_flatten = extractors[key].forward(th.as_tensor(subspace.sample()[None]).float()).shape[1]
        ...
        self.combined_layer = nn.Sequential(
            nn.Linear(total_concat_size, 192),  # 映射到192维度
        )

        # Update the features dim manually
        self._features_dim = 192  # 最终输出维度是192
...
class MY_Assist_NatureCNN(BaseFeaturesExtractor):
    """
    CNN from DQN Nature paper:
        Mnih, Volodymyr, et al.
        "Human-level control through deep reinforcement learning."
