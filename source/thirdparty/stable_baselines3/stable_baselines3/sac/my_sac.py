...
from stable_baselines3.common.expert_off_policy_algorithm import ExpertOffPolicyAlgorithm
from stable_baselines3.common.policies import BasePolicy, ContinuousCritic
from stable_baselines3.common.type_aliases import GymEnv, MaybeCallback, Schedule
from stable_baselines3.common.utils import get_parameters_by_name, polyak_update
from stable_baselines3.sac.policies import Actor, CnnPolicy, MlpPolicy, MultiInputPolicy, SACPolicy,ExpertMultiInputPolicy
import os 
from typing import Tuple, Dict
SelfSAC = TypeVar("SelfSAC", bound="MY_SAC")
...
class MY_SAC(ExpertOffPolicyAlgorithm):
    """
    ...
    def __init__(
        ...
        learning_starts: int = 256,#原256 现在36000
        batch_size: int = 256,
        tau: float = 0.005,
        gamma: float = 0.99,
        ...
        target_entropy: Union[str, float] = "auto",#auto
        use_sde: bool = False,
    ...
    def _setup_model(self) -> None:
        ...
        print(self.policy,self.policy.actor,self.policy.critic,self.policy.critic_target)
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        imitation_discount=1-self.num_timesteps/2e6
        if imitation_discount<0:
            imitation_discount=0
        for gradient_step in range(gradient_steps):
            ...
            # print(1)
            # print(type(replay_data.observations))
            # print(2)
            # print(replay_data.actions)
            # print(3)
            # print(type(replay_data.actions))
            # We need to sample because `log_std` may have changed between two gradient steps
            if self.use_sde:
                self.actor.reset_noise()

            # Action by the current actor for the sampled state
            actions_pi, log_prob = self.actor.action_log_prob(replay_data.observations)#action_pi是tanh之后的
            ...
            current_q_values = self.critic(replay_data.observations, replay_data.actions)#replay_data.actions是【-1，1】的
            ...
            #print("replay_data.expert_actions:",replay_data.expert_actions)
            ...
            actor_loss = (ent_coef * log_prob - min_qf_pi).mean()+ F.mse_loss(actions_pi, replay_data.expert_actions) *imitation_discount  # 添加行为克隆损失项
            #actor_loss = F.mse_loss(actions_pi, replay_data.expert_actions) *5  # 添加行为克隆损失项
...
class ExpertExperiencePool:
    ...
    def sample(self, batch_size: int) -> Tuple[Dict[str, th.Tensor], th.Tensor]:
        ...
        for key, state_array in self.state_dict.items():
            # 转换为GPU张量 
            expert_state[key] = th.from_numpy(state_array[indices]).float().to(self.device)
        
        # 构建动作张量（GPU张量）
        expert_action = th.from_numpy(self.action[indices]).float().to(self.device)
        
        return expert_state, expert_action

    def get_gpu_tensors(self) -> Tuple[Dict[str, th.Tensor], th.Tensor]:
        """获取整个经验池的GPU张量（谨慎使用，可能占用大量显存）"""
        expert_state = {}
        for key, state_array in self.state_dict.items():
            expert_state[key] = th.from_numpy(state_array[:self.current_size]).float().to(self.device)
        
        expert_action = th.from_numpy(self.action[:self.current_size]).float().to(self.device)
